...
`Auto` は現在の Obsidian の言語設定に追従します。`English`（英語）、`Japanese`（日本語）、`Korean`（韓国語）のいずれかに固定することもできます。
...
IGDB のスクリーンショットを vault 内へ保存します。

スクリーンショットは、設定した保存先ルート配下にゲームごとのサブフォルダを作って保存されます。
...
| `localScreenshot`       | 保存済みスクリーンショットのローカルパス文字列       |
| `localScreenshots`      | 保存済みスクリーンショットのローカルパス配列         |
...
- **翻訳**: 新しい UI 文字列を追加するときは、`src/locales/en.ts` とすべてのロケールファイルにキーを追加し、README にも行を追加してください。
- **ビルド**: `pnpm build`
- **リリース**: `pnpm release`（standard-version）— バージョンタグをプッシュします。GitHub Actions のワークフローがリリースアセットをビルドして添付します。

## Acknowledgements

このプロジェクトは [anpigon/obsidian-book-search-plugin](https://github.com/anpigon/obsidian-book-search-plugin) をベースに、IGDB のゲームメタデータ向けに作り直したものです。
